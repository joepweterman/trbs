...
def write_report(out_dir, profiles, front, paired, decomp, overhead_synth, overhead_pack, scored, counts):
    ...
    evidence = {
        ...
        "P3": f"median share of runtime that is not evaluation, k>=9: "
        f"{scored['P3']['median_share_not_evaluation_k9plus']}",
...
def main():
    ...
    report = write_report(
        OUT, profiles, front, paired, decomp, overhead_synth, overhead_pack, scored, counts
    )
    for name in ("P1", "P2", "P3", "P4"):
